...
//        return userDetails;
    ...
    public int getRescheduleCount() {
        ...
        try {
            ...
            con = DriverManager.getConnection(databaseUrl, databaseUsername, databasePassword);
            stmt = con.createStatement();
            ResultSet rst = stmt.executeQuery("SELECT COUNT(request_id) AS count FROM doctor_rescheduling_timetable");
            if (rst.next()) {
                incount = rst.getInt("count");
            }
            con.close();
    ...
    public List<ReschedulingDto> getReschedulingRequest() {
        ...
        try {
            ...
            con = DriverManager.getConnection(databaseUrl, databaseUsername, databasePassword);
            stmt = con.createStatement();
            ResultSet rst = stmt.executeQuery("select request_id,username,requested_date,requested_time,status from doctor_rescheduling_timetable;");
            while (rst.next()) {
                ReschedulingDto rescheduleRequest = new ReschedulingDto();
                rescheduleRequest.setRequest_id(rst.getInt("request_id"));
                rescheduleRequest.setDoctor_username(rst.getString("username"));
    ...
    public boolean updateReschedulingRequest(int request_id, String status) {
        ...
        try {
            ...
            con = DriverManager.getConnection(databaseUrl, databaseUsername, databasePassword);
            stmt = con.createStatement();
            stmt.executeUpdate("update doctor_rescheduling_timetable set status = '" + status + "' where request_id = " + request_id + ";");
            con.close();
            stmt.close();
